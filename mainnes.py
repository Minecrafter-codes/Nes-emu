...
def nesstart(rom_):
    ...
    try:
        ...
        try:
            ...
            logging.warning(f'pyximport is not available: {e}')
        print(f'rom: {rom}')
        ...
        nes = None
        #nes = NES("./roms/T.nes", sync_mode=SYNC_AUDIO, opengl=True)
        nes = NES(rom,                  # the rom file to load
                  ...
                  log_level=logging.WARNING,            # level of logging (logging is largely turned off by default)
                  opengl=True,              # use opengl for screen rendering
        ...
        print('Sorry a Fatal Error occured:')
        import traceback, sys, gc
        gc.collect()
        traceback.print_exc()
        with open('./restart/error.txt', 'w') as f:
            f.write(traceback.format_exc())
            f.close()
        print(f'NES settings: {nes}')
        sys.exit(1)
