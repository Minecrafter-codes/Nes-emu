...
from logger import plogging as plog
...
cdef class NESPPU:
    ...
    def __init__(self, cart=None, interrupt_listener=None, palette=None):
        plog.debug(f'PPU: INIT: Starting ppu')
        # Registers
        self.ppu_ctrl = 0
        self.ppu_mask = 0
        self.oam_addr = 0
    ...
    cdef void write_oam(self, unsigned char* data):
        ...
        if not count == a:
            a = count
            plog.debug(f'PPU: write_oam (voided): {OAM_SIZE_BYTES}')
        for i in range(OAM_SIZE_BYTES):
            self.oam[i] = data[i]
            count = OAM_SIZE_BYTES
    ...
    cdef unsigned char read_register(self, int register):
        ...
        #plog.debug(f'PPU: read_register: {register}')
    ...
    cdef void write_register(self, int register, unsigned char value):
        ...
        #plog.debug(f'PPU: write_register: {register}')
        if register == PPU_CTRL:
            # write only
            ...
            if self.ignore_ppu_ctrl:
                if self.cycles_since_reset < 29658:
                    plog.debug('PPU: ignoring ppu control')
                    return
                else:
    ...
    cdef int run_cycles(self, int num_cycles):
        ...
        #plog.debug(f'PPU: run_cycles. Vblank_started: {vblank_started}')
