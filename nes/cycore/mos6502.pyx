...
cdef class MOS6502:
    ...
    """
    def __init__(self, memory, undocumented_support_level=1, aax_sets_flags=False, stack_underflow_causes_exception=True, ignore_halt=True):

        # memory is user-supplied object with read and write methods, allowing for memory mappers, bank switching, etc.
        try:
            self.memory = memory
        except:
            logger.warning('No memory')
        ...
        self.ignore_halt = ignore_halt

        # reset the cpu
        try:
            self.reset()
        except:
             logger.error('No reset!!!')
            ...
            for mode, instr in instr_set.modes.items():
                if type(instr.bytecode) == list:
                    ...
                    for bytecode in instr.bytecode:
                        logger.info(f'Cpu: Bytecode: {bytecode}')
                    ...
                    logger.info(f'Cpu: Bytecode: {instr.bytecode}')
        ...
        for _, instr_set in INSTRUCTION_SET.items():
            for mode, instr in instr_set.modes.items():
                ...
                for bytecode in bytecodes:
                    logger.info(f'Cpu: Bytes: {instr.size_bytes}')
    ...
    def format_instruction(self, instr, data, caps=True):
        ...
        logger.info(f'Cpu: Formatted Instructions: {line}')
        return line
    ...
    cdef int trigger_nmi(self):
        ...
        logger.info(f'Cpu: Trigger_nmi: {INTERRUPT_REQUEST_CYCLES}')
    ...
    cdef int trigger_irq(self):
        ...
        if not self.I:
            ...
            self.cycles_since_reset += INTERRUPT_REQUEST_CYCLES
            logger.info(f'Cpu: Trigger_irq: {INTERRUPT_REQUEST_CYCLES}')
            return INTERRUPT_REQUEST_CYCLES
        else:
            #logger.info(f'Cpu: Trigger_irq: {0}')
            return 0  # ignored!
    ...
    cdef pop_stack(self):
        ...
        if self.SP == 0xFF and self.stack_underflow_causes_exception:
            logger.fatal(f'Cpu: pop_stack: self.SP = {self.SP} .Stack Underflow')
            raise OverflowError("Stack underflow")
            
        if self.SP == 0xFF and not self.stack_underflow_causes_exception:
            logger.fatal(f'Cpu: pop_stack: self.SP = {self.SP} .Stack Underflow Ignored')
            #self.reset()
    ...
    def _kil(self, _, __):
        ...
        if self.undocumented_support_level >= 1:
            if self.ignore_halt:
                warnings.warn('KIL instruction Ignored')
            if not self.ignore_halt:
