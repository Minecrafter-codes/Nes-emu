...
from logger import plogging as plogger
    ...
    def __init__(self,
        ...
        if prg_rom_data:
            ...
            if self.prg_rom_size not in [16 * 1024, 32 * 1024]:
                raise ValueError(f"Cart 0 program rom size should be exactly 16 or 32kB. Current: {self.prg_rom_size}")
...
cdef class NESCart1(NESCart):
    ...
    cdef void _write_shift(self, int address, unsigned char value):
        ...
        if self.shift_ctr == 5:
            ...
            if M1_CTRL_REG_START <= address < M1_CHR_REG_0_START:
                ...
                plogger.debug("UNMATCHED SHIFT FINISH {:04X}".format(address))
                #logging.log(logging.DEBUG, "UNMATCHED SHIFT FINISH {:04X}".format(address))
    ...
    cdef void _set_nametable_mirror_pattern(self):
        ...
        else: plogger.debug(f'Mirror_Pattern Expected 0-3. got {mirror_pattern}') #logging.log(logging.DEBUG, f'Mirror_Pattern Expected 0-3. got {mirror_pattern}')
...
cdef class NESCart4(NESCart):
    ...
    cdef void irq_tick(self):
        ...
        #sys.stdout.write("IRQ counter = {}\n".format(self.irq_counter))
        #clogger.DEBUG(f"IRQ counter = {self.irq_counter}")
        #logging.log(logging.DEBUG, "IRQ counter = {}".format(self.irq_counter), extra={"source": "Cart"})
