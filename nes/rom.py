...
def write(msg):
    print(f'{float(time.time() - start_time)}: {msg}')
...
class ROM:
    ...
    def load(self, filename, ):
        ...
        with open(filename, "rb") as f:
            ...
            if len(self.misc_rom_data) > 0:
                write("WARNING: MISC ROM DATA IS NOT EMPTY")
    ...
    def decode_header(self, nesheader):
        ...
        if not (nesheader[0] == 'N' and nesheader[1] == 'E' and nesheader[2] == 'S' and nesheader[3] == 0x1A):
            ...
            write("WARNING: Invalid .nes file header")
        ...
        if not self.nes2:
            ...
            if self.verbose:
                write("iNES (v1) Header")
        else:
            # NES 2.0 format
            # https://wiki.nesdev.com/w/index.php/NES_2.0
            if self.verbose:
                write("NES 2.0 Header")
            # header byte 8
        ...
        if self.verbose:
            write("Mapper: {}".format(self.mapper_id))
            write("prg_ram_bytes: {}".format(self.prg_ram_bytes))
            write("chr_ram_bytes: {}".format(self.chr_ram_bytes))
            write("prg_rom_bytes: {}".format(self.prg_rom_bytes))
            write("chr_rom_bytes: {}".format(self.chr_rom_bytes))
            write("mirror pattern: {}".format(self.mirror_pattern))
    ...
    def get_cart(self, interrupt_listener):
        ...
        if self.py_compatibility_mode:
            if self.mapper_id==0:
                ...
                write("Mapper {} not currently supported in py_compatibility_mode".format(self.mapper_id))
        ...
        if self.mapper_id == 0:
        ...
        elif self.mapper_id == 3 or self.mapper_id == 225:
            # MMC 3
            return NESCart1(prg_rom_data=self.prg_rom_data,
                            chr_rom_data=self.chr_rom_data,
                            prg_ram_size_kb=self.prg_ram_bytes / 1024,
            ...
            #if self.mapper_id > 4:
            #    '''Note: Python should only crash at mapper 3 because is not supported maybe will add support in future '''
            #    valid_mapper = "0, 1, 2, 4"
            #    raise MapperError(f'Mapper above 4 is no longer a valid rom mapper. Try using another emulator that is supported for this #mapper. \nCurrent Mapper: {self.mapper_id}, Expected Valid Mapper: {valid_mapper}')
            #print("Mapper {} not currently supported".format(self.mapper_id))
            warnings.warn(f"Mapper {self.mapper_id} not currently supported. This may cause unexpected bahavior")
            #raise MapperError('Invalid Mapper')
