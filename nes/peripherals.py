...
# create plogger
plogger = logging.getLogger('Peripherals')
...
#More
logger = logging.getLogger('OpenGL.GL.shaders')
logger.setLevel(logging.DEBUG)
logger.addHandler(consoleHandler)

plogger.debug('started plogger')

# would like to make this not depend on pygame
from tkinter import messagebox
try:
    plogger.debug('Starting Pygame')
    import pygame
    import pygame.freetype
    plogger.debug('Started Pygame')
    has_pygame = True
except ImportError:
    has_pygame = False

try:
    plogger.debug('Starting PyOpenGL')
    from OpenGL.GL import *
    
    from OpenGL.GL import shaders
    plogger.debug('Started PyOpenGL')
    has_opengl = True
except ImportError:
    has_opengl = False
...
def drawText(x, y, text):  
    try:
        ...
        textSurface = font.render(text, True, (255, 255, 66, 255), (0, 66, 0, 255)).convert_alpha()
        textData = pygame.image.tostring(textSurface, "RGBA", True)
        glWindowPos2d(x, y)
        glDrawPixels(textSurface.get_width(), textSurface.get_height(), GL_RGBA, GL_UNSIGNED_BYTE, textData)
    except Exception as e:
        plogger.warning(f'Failed to draw text. Reason: {e}')
...
class Screen(ScreenBase):
    ...
    def __init__(self, ppu, scale=3, vsync=False, vertical_overscan=False, horizontal_overscan=False, nametable_panel=False, py_compatibility_mode=False):
        try:
            ...
            plogger.info('Created Pygame window')
        except Exception as e:
            messagebox.showerror('Error 4:', f'Failed to Create Window: {e}')
    ...
    def _render_text(self, surf):
        try:
            ...
            plogger.error('{e}')
            messagebox.showerror('Error 1:', f'Failed to render text: {e}')
            pygame.quit()
    ...
    def show(self):
        try:
            ...
            traceback.print_exc()
            messagebox.showerror('Error 2:', f'Failed to Update or show: {e}')
            pygame.quit()
...
class ScreenGL(ScreenBase):
    ...
    def __init__(self, ppu, scale=3, vsync=False, vertical_overscan=False, horizontal_overscan=False):
        try:
            ...
            plogger.debug('Created Pygame gl window')
        except Exception as e:
            traceback.print_exc()
            messagebox.showerror('Error 3:', f'Failed to Create OpenGl Window: {e}')
    ...
    def show(self):
        try:
            ...
            traceback.print_exc()
            messagebox.showerror('Error 5:', f'Failed to Update or Show Gl: {e}')
            pygame.quit()
...
class KeyboardController(ControllerBase):
    ...
    def __init__(self, active=True, key_map=None):
        try:
            ...
            plogger.error(e)
            messagebox.showerror('Error 8:', f'Failed to initialize KeyboardController: {e}')
...
P = A
HOME = Reset CPU
DELETE = Restart Emulator''')
            #return exec(open("controls.txt").read())
            return {
            ...
            traceback.print_exc()
            messagebox.showerror('Error 7:', f'Failed to create default Keymap: {e}')
    ...
    def update(self):
        ...
        try:
            ...
            traceback.print_exc()
            messagebox.showerror('Error 6:', f'Failed to Read Keyboard: {e}')
